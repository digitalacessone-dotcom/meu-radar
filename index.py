...
def get_weather_info(lat, lon):
    try:
        ...
        curr = resp['current']
        mapping = {0: "CLEAR SKY", 1: "FEW CLOUDS", 2: "SCATTERED", 3: "OVERCAST", 45: "FOGGY", 61: "RAIN"}
        return {"temp": f"{int(curr['temperature_2m'])}C", "sky": mapping.get(curr['weather_code'], "STABLE")}
    except:
...
def radar():
    try:
        ...
        if test:
            return jsonify({"flight": {"icao": "E4953E", "reg": "PT-MDS", "call": "TEST777", "airline": "LOCAL TEST", "color": "#34a8c9", "dist": 10.5, "alt": 35000, "spd": 850, "hd": 120, "date": now_date, "time": now_time, "route": "GIG-MIA", "vrate": -1200, "squawk": "2241", "cat": "HEAVY"}, "weather": w, "date": now_date, "time": now_time})
            ...
            for s in data:
                ...
                if slat and slon:
                    ...
                    if d <= RADIUS_KM:
                        ...
                        proc.append({
                            "icao": s.get('hex', 'UNK').upper(), "reg": s.get('r', 'N/A').upper(), "call": call, 
                            "airline": airline, "color": color, "dist": round(d, 1), 
                            "alt": int(s.get('alt_baro', 0) if s.get('alt_baro') != "ground" else 0), 
                            "spd": int(s.get('gs', 0) * 1.852), "hd": int(s.get('track', 0)), 
                            "date": now_date, "time": now_time, "route": s.get('route', "--- ---"),
                            "vrate": s.get('baro_rate', 0), "squawk": s.get('squawk', '0000'),
                            "cat": "HEAVY" if s.get('category','').startswith('A5') else "MEDIUM"
                        })
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { background: var(--bg); font-family: sans-serif; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100dvh; margin: 0; perspective: 1500px; overflow: hidden; }
        ...
        #bc { width: 110px; height: 35px; opacity: 0.15; cursor: pointer; }
        .ticker { width: 310px; height: 32px; background: #000; border-radius: 6px; margin-top: 15px; display: flex; align-items: center; justify-content: center; color: var(--gold); font-family: monospace; font-size: 11px; letter-spacing: 1px; }
...
<body onclick="handleFlip(event)">
    <div id="ui">
        <input type="text" id="in" placeholder="CITY NAME (EX: LONDON)">
    ...
    <script>
        let pos = null, act = null, isTest = false, tickerMsg = [], tickerIdx = 0, lastDist = null;
        ...
        async function update() {
            ...
            try {
                ...
                if(d.flight) {
                    const f = d.flight;
                    let trend = f.dist < (lastDist || 999) ? "CLOSING IN" : "MOVING AWAY";
                    lastDist = f.dist;
                    ...
                    applyFlap('b-alt', f.alt + " FT"); applyFlap('b-spd', f.spd + " KMH");
        ...
        function hideUI() { document.getElementById('ui').classList.add('hide'); setTimeout(() => { update(); setInterval(update, 12000); }, 800); }
