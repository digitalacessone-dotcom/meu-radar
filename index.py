...
# Configurações V94 - Resiliência Vercel & Estabilidade
RADIUS_KM = 200 
DEFAULT_LAT = -22.9068
DEFAULT_LON = -43.1729
...
def fetch_aircrafts(lat, lon):
    # Lista simplificada para evitar timeout no Vercel
    urls = [
...
def radar():
    try:
        ...
        now_dt = get_time_local()
        now_date = now_dt.strftime("%d %b %Y").upper()
        now_time = now_dt.strftime("%H.%M")
        w = get_weather(lat, lon)
        
        if test:
            return jsonify({"flight": {"icao": "E4953E", "reg": "PT-MDS", "call": "TEST777", "airline": "LOCAL TEST", "color": "#34a8c9", "dist": 10.5, "alt": 35000, "spd": 850, "hd": 120, "date": now_date, "time": now_time, "route": "GIG-MIA", "eta": 1, "kts": 459}, "weather": w})
                ...
                if slat and slon:
                    ...
                    if d <= RADIUS_KM:
                        ...
                        proc.append({
                            ...
                            "dist": round(d, 1), "alt": int(s.get('alt_baro', 0) if s.get('alt_baro') != "ground" else 0),
        ...
        return jsonify({"flight": None, "error": "Backend Busy"})
        ...
        async function update() {
            ...
            try {
                ...
                if(d.flight) {
                    ...
                    applyFlap('tk', tickerMsg[0], true);
                } else { 
                    tickerMsg = [`SEARCHING TRAFFIC...`, `ESTIMATED TEMP: ${d.weather.temp}`];
                    applyFlap('tk', tickerMsg[0], true);
