AS LETRAS ESATO PARANDO RAPIDO DEMAIS. PREISAM PARAR DE FORMA ALEATORIA COM TEMPOS DE PARADA DESENCOTRADOS E COM TEMPO MAIOR. CADA LETRA TEM TEMPO DIFERENTE PARA PARADA INCLUSIVE OS ESAPCOS VAZIOS

ATRAS SELO FICOU GRANDE E TAMPA O CARIMBO. TEM QUE SER PEQUENO, ELEGANTE E DISCRETO.

AS LETRAS AMARELAS EM BAIXO NA FAIXA PRETA PERDEU ESPACO ENTRE AS PALAVRAS.

A DATA E HORA DESFORMATOU DA VERSAO ESTAVEL

Lado Esquerdo (Stub): FICA preto (#000), COM LETRAS DOURADAS APENAS O LADO ESQUERDO SE LOCALIZAR AERONAVE MILITAR OU RARA. .

Selo Metálico Dourado: Adicionado um selo de autenticidade no verso do cartão, posicionado na área branca.

Persistência (Histórico) DE SELO: Voos reais MILITARES E RAROS detectados são salvos no localStorage do navegador para manter a coleção do usuário (SELOS). Voos de teste são ignorados pela gravação de histórico.

PARA FINS DE TESTE, INCLUIR A AERONAVE BLACKBIRD NO MODO TESTE MISTURADO COM AVIOES COMERCIAIS.

SEGUE SCRIPIT ESTAVEL: 
