...
def get_weather(lat, lon):
    """Busca telemetria meteorológica local."""
    try:
...
def fetch_aircrafts(lat, lon):
    """Busca dados de voo com rotação de endpoints para evitar rate limit."""
    endpoints = [
        ...
        if test:
            return jsonify({
                "flight": {
                    ...
                    "color": "#34a8c9", "dist": 10.5, "alt": 35000, "spd": 850, "hd": 120, 
                    "date": now_date, "time": now_time, "route": "GIG-MIA", "eta": 1, 
                    "kts": 459, "vrate": 1500
                }, 
                ...
                if slat and slon:
                    ...
                    if d <= RADIUS_KM:
                        ...
                        proc.append({
                            ...
                            "eta": eta, "vrate": int(s.get('baro_rate', 0))
                        })
            if proc: 
...
<body onclick="handleFlip(event)">
    ...
    <script>
        ...
        // Alternância de labels a cada 15 segundos
        setInterval(() => {
            if(act) {
                ...
                document.getElementById('spd-label').innerText = toggleState ? "GROUND SPEED" : "KNOTS (GS)";
        ...
        async function update() {
            ...
            try {
                ...
                if(d.flight) {
                    ...
                    tickerMsg = [`V.RATE: ${f.vrate} FPM`, `STATUS: ${vStatus}`, proximity, `TEMP: ${weather.temp} | ${weather.sky}`];
                    ...
                    tickerMsg = [g, "SEARCHING TRAFFIC...", `WEATHER: ${weather.sky}`];
        ...
        navigator.geolocation.getCurrentPosition(p => { pos = {lat:p.coords.latitude, lon:p.coords.longitude}; hideUI(); }, () => { applyFlap('tk', 'MANUAL SEARCH REQUIRED', true); }, { timeout: 6000 });
    </script>
</body>
</html>
''')
