...
# Configurações V97.1 - Estabilidade Serverless
RADIUS_KM = 190
DEFAULT_LAT = -22.9068
DEFAULT_LON = -43.1729
...
def get_weather(lat, lon):
    try:
        ...
        return {
            "temp": f"{int(curr.get('temperature_2m', 0))}C", 
            "sky": get_weather_desc(curr.get('weather_code', 0)), 
            "vis": f"{vis_km}KM"
        }
    except Exception:
...
def radar():
    try:
        ...
        if test:
            return jsonify({
                "flight": {
                    "icao": "E4953E", "reg": "PT-MDS", "call": "TEST777", 
                    "airline": "LOCAL TEST", "color": "#34a8c9", "dist": 10.5, 
                    "alt": 35000, "spd": 850, "hd": 120, "date": now_date, 
                    "time": now_time, "route": "GIG-MIA", "eta": 1, 
                    "kts": 459, "vrate": 1500
                }, 
        ...
        # Retorna JSON vazio em vez de erro 500
        ...
        <div class="face back">
            <div style="height:100%; border:1px dashed #ccc; border-radius:15px; padding:20px; display:flex; flex-direction:column;">
                ...
                <div style="border: 3px double var(--blue-txt); color: var(--blue-txt); padding: 10px; border-radius: 10px; transform: rotate(-10deg); align-self: center; margin-top: 20px; text-align: center; font-weight: 900;">
                    ...
                    <div style="font-size:8px; margin-top:5px;">RADAR CONTACT V97.1</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function update() {
            ...
            try {
                ...
                if (d.error) { console.log("API Error:", d.error); return; }
                ...
                if(d.flight) {
                    ...
                    tickerMsg = [g, "SEARCHING TRAFFIC...", g, `TEMP: ${weather.temp}`, g, `VIS: ${weather.vis}`, g, weather.sky];
            ...
            } catch(e) { console.log("Update Fail"); }
        }
        ...
        function startSearch() {
            ...
            if(v === "TEST") { isTest = true; pos = {lat:-22.9, lon:-43.1}; hideUI(); }
                ...
                .then(r=>r.json())
                .then(d=>{ if(d[0]) { pos = {lat:parseFloat(d[0].lat), lon:parseFloat(d[0].lon)}; hideUI(); } }); 
            }
        }
...
# Para o Vercel, o objeto 'app' deve estar disponível no escopo global.
# Removido app.run para evitar conflitos no ambiente serverless.
