...
# Configurações V106 - Raio 190km e Calibração de Dots
RADIUS_KM = 190 
DEFAULT_LAT = -22.9068
DEFAULT_LON = -43.1729
...
def radar():
    try:
        ...
        if test:
            return jsonify({"flight": {"icao": "E4953E", "reg": "PT-MDS", "call": "TEST777", "airline": "LOCAL TEST", "color": "#34a8c9", "dist": 15.5, "alt": 35000, "spd": 850, "hd": 120, "date": now_date, "time": now_time, "route": "GIG-MIA", "eta": 2, "kts": 459, "v_rate": -1500, "sqk": "3314"}, "weather": w})
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .ticker { width: 310px; height: 35px; background: #000; border-radius: 6px; margin-top: 15px; display: flex; align-items: center; justify-content: center; color: var(--gold); font-family: monospace; font-size: 10px; letter-spacing: 0.5px; border: 1px solid #333; }
        .ghost .stub { background: #555 !important; }
        .ghost .main { filter: grayscale(1); opacity: 0.7; }
...
<body onclick="handleFlip(event)">
        ...
        <div class="face front" id="card-front">
            ...
            <div class="main">
                ...
                <div style="display:flex; justify-content:space-between; align-items:flex-end;">
                    ...
                    <div class="date-visual"><div id="f-date">-- --- ----</div><div id="f-time" style="font-size:22px;">--.--</div><img id="bc" src="https://bwipjs-api.metafloor.com/?bcid=code128&text=WAITING"></div>
                </div>
            </div>
        </div>
        <div class="face back">
            <div style="height:100%; border:1px dashed #ccc; border-radius:15px; padding:20px; display:flex; flex-direction:column; justify-content:space-between;">
                ...
                <div style="border:3px double var(--blue-txt); color:var(--blue-txt); padding:10px; border-radius:10px; transform:rotate(-10deg); text-align:center; font-weight:900; align-self:center;">
                    ...
                    <div style="font-size:8px; margin-top:5px;">RADAR CONTACT V106</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function applyFlap(id, text) {
            ...
            el.innerHTML = '';
            [...target].forEach(c => {
                const s = document.createElement('span'); s.className='char'; s.innerHTML='&nbsp;'; el.appendChild(s);
                let count=0, max=15+Math.floor(Math.random()*15);
                const inv = setInterval(() => {
                    s.innerText = chars[Math.floor(Math.random()*chars.length)];
                    if(count++ >= max) { clearInterval(inv); s.innerText = c===' '?'':c; }
                }, 40);
            });
        }
        ...
        async function update() {
            ...
            try {
                ...
                if(d.flight) {
                    ...
                    // Calibração Dots (Esquerda p/ Direita)
                    const dists = [190, 150, 110, 70, 30];
                    for(let i=1; i<=5; i++) {
                        document.getElementById('d'+i).className = f.dist <= dists[i-1] ? 'sq on' : 'sq';
                    }
                    
                    let v_st = f.v_rate > 150 ? "SUBINDO" : (f.v_rate < -150 ? "DESCENDO" : "ESTÁVEL");
                    let d_st = f.dist < lastDist ? "APROXIMANDO" : "AFASTANDO";
                    lastDist = f.dist;

                    document.getElementById('tk').innerText = `V.RATE: ${f.v_rate} FPM (${v_st}) • SQK: ${f.sqk} • ${d_st} • TEMP: ${w.temp} • VIS: ${w.vis} • ${w.sky}`;
                    act = f;
                } else if(act) {
                    // Modo Ghost
                    document.getElementById('card-scene').classList.add('ghost');
                    document.getElementById('tk').innerText = "SIGNAL LOST / GHOST MODE ACTIVE";
                    for(let i=1; i<=5; i++) document.getElementById('d'+i).classList.remove('on');
                } else {
                    // Scanning
