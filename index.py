...
# Configurações V102.0 - Rare Detection & Blackbird Test
RADIUS_KM = 190 
DEFAULT_LAT = -22.9068
DEFAULT_LON = -43.1729
...
def radar():
    try:
        ...
        if test:
            # Aeronave Militar para Teste: SR-71 Blackbird
            return jsonify({"flight": {"icao": "SR71", "reg": "61-7972", "call": "BLACKBIRD", "airline": "US AIR FORCE", "color": "#000000", "dist": 10.5, "alt": 85000, "spd": 3529, "hd": 270, "date": now_date, "time": now_time, "route": "AREA51-HIDDEN", "eta": 1, "kts": 1905, "vrate": 0, "rare": True}, "weather": w, "date": now_date, "time": now_time})
            ...
            for s in data:
                ...
                if slat and slon:
                    ...
                    if d <= RADIUS_KM:
                        ...
                        # Lógica de raridade baseada em mil/dbflags
                        if s.get('mil') == "1" or s.get('dbflags') == 1:
                            airline, color, is_rare = "SPECIAL OPS", "#000000", True
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .stub { height: 32%; background: var(--brand); color: #fff; padding: 20px; display: flex; flex-direction: column; justify-content: center; transition: all 0.5s; }
        
        /* Selo Metálico Dourado */
        .rare-seal { 
            position: absolute; bottom: 20px; right: 20px; width: 55px; height: 55px; 
            border-radius: 50%; background: radial-gradient(circle, #f9f9f9 0%, #d4af37 45%, #8a6d3b 100%);
            border: 1.5px solid #b8860b; display: none; align-items: center; justify-content: center;
            box-shadow: inset 0 0 5px rgba(0,0,0,0.2), 0 4px 8px rgba(0,0,0,0.3);
            font-size: 8px; font-weight: 900; color: #4a3511; text-align: center; line-height: 1; text-transform: uppercase;
        }
...
<body onclick="handleFlip(event)">
    ...
    <div class="scene" id="card">
        <div class="face front">
            <div class="stub" id="stb">
                <div id="scan-label" style="font-size:7px; font-weight:900; opacity:0.7;">RADAR SCANNING</div>
        ...
        <div class="face back">
            <div style="height:100%; border:1px dashed #ccc; border-radius:15px; padding:20px; display:flex; flex-direction:column; position: relative;">
                ...
                <div style="border: 3px double var(--blue-txt); color: var(--blue-txt); padding: 10px; border-radius: 10px; transform: rotate(-10deg); align-self: center; margin-top: 20px; text-align: center; font-weight: 900;">
                    ...
                    <div style="font-size:8px; margin-top:5px;">RADAR CONTACT V102.0</div>
                </div>
                <div id="seal" class="rare-seal">LIMITED<br>EDITION</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let toggleState = true; let isGhost = false;
        let tickerMsg = [], tickerIdx = 0;
        let audioCtx = null;
        let fDate = "-- --- ----", fTime = "--.--";
        const chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ.- ";

        // Função de Histórico de Raros
        function saveRare(f) {
            if(isTest) return; // Não salvar se for modo teste
            let history = JSON.parse(localStorage.getItem('rare_history') || '[]');
            if(!history.some(h => h.icao === f.icao)) {
                history.push({icao: f.icao, reg: f.reg, date: f.date, airline: f.airline});
                localStorage.setItem('rare_history', JSON.stringify(history));
                console.log("Rare aircraft saved to history:", f.icao);
            }
        }
        ...
        function applyFlap(id, text, isTicker = false) {
            ...
            [...target].forEach((char) => {
                ...
                let count = 0, max = 20 + Math.floor(Math.random() * 25); 
        ...
        async function update() {
            ...
            try {
                ...
                const d = await r.json(); weather = d.weather;
                
                if(d.flight) {
                    const f = d.flight; isGhost = false;
                    const stub = document.getElementById('stb');
                    const seal = document.getElementById('seal');
                    
                    // Lógica Visual Rara
                    if(f.rare) {
                        stub.style.background = "#000000";
                        stub.style.color = "var(--gold)";
                        document.getElementById('scan-label').innerText = "RARE CONTACT IDENTIFIED";
                        seal.style.display = "flex";
                        saveRare(f); // Salva no histórico (validando se não é teste internamente)
                    } else {
                        stub.style.background = f.color;
                        stub.style.color = "#fff";
                        document.getElementById('scan-label').innerText = "RADAR SCANNING";
                    ...
                    act = f;
                    tickerMsg = [f.rare ? "RARE SPECIMEN" : "CONTACT ESTABLISHED", `ALT: ${f.alt} FT`, `SPD: ${f.spd} KMH` ];
                }
            } catch(e) {}
        }
