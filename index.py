# -*- coding: utf-8 -*-
from flask import Flask, jsonify, request, render_template_string
import requests
import math
import random
from datetime import datetime, timedelta

app = Flask(__name__)

# Configurações V107.0 - Military Rare Edition
RADIUS_KM = 190 
DEFAULT_LAT = -22.9068
DEFAULT_LON = -43.1729

def get_time_local():
    return datetime.utcnow() - timedelta(hours=3)

def is_rare(icao, call):
    # Lista expandida para detecção militar e rara
    milit_prefixes = ('MIL', 'NAVY', 'AF1', 'FORCA', 'RCH', 'VADER', 'SR71', 'B2', 'F22', 'F35', 'U2', 'K35R', 'R135', 'ROMA')
    rare_icaos = ["E4953E", "AE01CE", "ADFEB7", "AE0808"]
    return call.upper().startswith(milit_prefixes) or icao.upper() in rare_icaos

def get_weather_desc(code):
    mapping = {0: "CLEAR SKY", 1: "FEW CLOUDS", 2: "SCATTERED", 3: "OVERCAST", 45: "FOG", 61: "RAIN", 80: "SHOWERS"}
    return mapping.get(code, "CONDITIONS OK")

def get_weather(lat, lon):
    try:
        url = f"https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&current=temperature_2m,weather_code,visibility"
        resp = requests.get(url, timeout=5).json()
        curr = resp['current']
        vis_km = int(curr.get('visibility', 10000) / 1000)
        return {"temp": f"{int(curr['temperature_2m'])}C", "sky": get_weather_desc(curr['weather_code']), "vis": f"{vis_km}KM"}
    except:
        return {"temp": "--C", "sky": "METAR ON", "vis": "--KM"}

def fetch_aircrafts(lat, lon):
    url = f"https://api.adsb.lol/v2/lat/{lat}/lon/{lon}/dist/200"
    headers = {'User-Agent': 'Mozilla/5.0'}
    try:
        r = requests.get(url, headers=headers, timeout=5)
        if r.status_code == 200: return r.json().get('aircraft', [])
    except: pass
    return []

@app.route('/api/radar')
def radar():
    try:
        lat = float(request.args.get('lat', DEFAULT_LAT))
        lon = float(request.args.get('lon', DEFAULT_LON))
        test = request.args.get('test', 'false').lower() == 'true'
        local_now = get_time_local()
        now_date = local_now.strftime("%d %b %Y").upper()
        now_time = local_now.strftime("%H.%M")
        w = get_weather(lat, lon)
        
        if test:
            # Rotação de Teste: 50% chance de ser o Blackbird (Raro)
            if random.random() > 0.5:
                f = {"icao": "SR71", "reg": "61-7972", "call": "BLACKBIRD", "airline": "STRATOSPHERE", "color": "#000", "dist": 42.1, "alt": 85000, "spd": 3530, "hd": 315, "date": now_date, "time": now_time, "route": "EDW-GIG", "rare": True, "vrate": 4500, "kts": 1900, "eta": 1}
            else:
                f = {"icao": "E4953E", "reg": "PT-MDS", "call": "TEST777", "airline": "LOCAL TEST", "color": "#34a8c9", "dist": 10.5, "alt": 35000, "spd": 850, "hd": 120, "date": now_date, "time": now_time, "route": "GIG-MIA", "rare": False, "vrate": -1500, "kts": 459, "eta": 2}
            return jsonify({"flight": f, "weather": w, "isTest": True})
        
        data = fetch_aircrafts(lat, lon)
        found = None
        if data:
            proc = []
            for s in data:
                slat, slon = s.get('lat'), s.get('lon')
                if slat and slon:
                    d = 6371 * 2 * math.asin(math.sqrt(math.sin(math.radians(slat-lat)/2)**2 + math.cos(math.radians(lat)) * math.cos(math.radians(slat)) * math.sin(math.radians(slon-lon)/2)**2))
                    if d <= RADIUS_KM:
                        icao = s.get('hex', 'UNK').upper()
                        call = (s.get('flight') or s.get('call') or 'N/A').strip().upper()
                        spd_kts = int(s.get('gs', 0))
                        spd_kmh = int(spd_kts * 1.852)
                        rare_status = is_rare(icao, call)
                        
                        # Definição de Cores (Stub)
                        color = "#000" if rare_status else "#444"
                        if not rare_status:
                            if call.startswith(("TAM","JJ","LA")): color = "#E6004C"
                            elif call.startswith(("GLO","G3")): color = "#FF6700"
                            elif call.startswith(("AZU","AD")): color = "#004590"

                        proc.append({
                            "icao": icao, "reg": s.get('r', 'N/A').upper(), "call": call, 
                            "airline": "PRIVATE" if not call.startswith(("TAM","GLO","AZU")) else ("LATAM" if "TAM" in call else "AIRLINE"),
                            "color": color, "dist": round(d, 1), "alt": int(s.get('alt_baro', 0) if s.get('alt_baro') != "ground" else 0),
                            "spd": spd_kmh, "kts": spd_kts, "hd": int(s.get('track', 0)),
                            "date": now_date, "time": now_time, "route": s.get('route', "--- ---"),
                            "vrate": int(s.get('baro_rate', 0)), "rare": rare_status, "eta": round((d/(spd_kmh or 1))*60)
                        })
            if proc: found = sorted(proc, key=lambda x: x['dist'])[0]
        return jsonify({"flight": found, "weather": w, "isTest": False})
    except: return jsonify({"flight": None})

@app.route('/')
def index():
    return render_template_string('''
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover">
    <style>
        :root { --gold: #FFD700; --bg: #0b0e11; --brand: #444; --blue-txt: #34a8c9; }
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        body { background: var(--bg); font-family: -apple-system, sans-serif; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100dvh; margin: 0; perspective: 1500px; overflow: hidden; }
        
        #ui { width: 280px; display: flex; gap: 6px; margin-bottom: 12px; z-index: 500; transition: opacity 0.8s; }
        #ui.hide { opacity: 0; pointer-events: none; }
        input { flex: 1; padding: 12px; border-radius: 12px; border: none; background: #1a1d21; color: #fff; font-size: 11px; outline: none; }
        button { background: #fff; border: none; padding: 0 15px; border-radius: 12px; font-weight: 900; }

        .scene { width: 300px; height: 460px; position: relative; transform-style: preserve-3d; transition: transform 0.8s; }
        .scene.flipped { transform: rotateY(180deg); }
        .face { position: absolute; width: 100%; height: 100%; backface-visibility: hidden; border-radius: 20px; background: #fff; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
        .face.back { transform: rotateY(180deg); background: #f4f4f4; padding: 15px; }

        .stub { height: 32%; background: var(--brand); color: #fff; padding: 20px; display: flex; flex-direction: column; justify-content: center; transition: 0.5s; position: relative; }
        
        /* Estilo para Raros */
        .rare-active .stub { background: #000 !important; color: var(--gold) !important; }
        .rare-active .sq.on { background: var(--gold); box-shadow: 0 0 15px var(--gold); border-color: var(--gold); }

        .dots-container { display: flex; gap: 4px; margin-top: 8px; }
        .sq { width: 10px; height: 10px; border: 1.5px solid rgba(255,255,255,0.3); background: rgba(0,0,0,0.2); border-radius: 2px; }
        .sq.on { background: #fff; }
        
        .perfor { height: 2px; border-top: 5px dotted #ccc; position: relative; background: #fff; }
        .perfor::before, .perfor::after { content:""; position:absolute; width:30px; height:30px; background:var(--bg); border-radius:50%; top:-15px; }
        .perfor::before { left:-25px; } .perfor::after { right:-25px; }

        .main { flex: 1; padding: 20px; display: flex; flex-direction: column; justify-content: space-between; }
        .flap { font-family: monospace; font-size: 18px; font-weight: 900; color: #000; height: 24px; display: flex; gap: 1px; }
        .char { width: 14px; height: 22px; background: #f0f0f0; border-radius: 3px; display: flex; align-items: center; justify-content: center; }
        
        .gold-seal { 
            position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-15deg);
            width: 100px; height: 100px; border-radius: 50%;
            background: radial-gradient(circle, #fff7ad 0%, #ffa700 50%, #8a5c00 100%);
            border: 2px solid #b38600; box-shadow: 0 4px 15px rgba(0,0,0,0.2);
            display: none; align-items: center; justify-content: center; 
            font-size: 9px; font-weight: 900; color: #5c3d00; text-align: center; line-height: 1.2;
            z-index: 10;
        }
        .rare-active .gold-seal { display: flex; }

        .ticker { width: 310px; height: 32px; background: #000; border-radius: 6px; margin-top: 15px; display: flex; align-items: center; justify-content: center; color: var(--gold); font-family: monospace; font-size: 11px; letter-spacing: 1px; white-space: pre; overflow: hidden; }
        #bc { width: 100px; height: 30px; opacity: 0.2; margin-top: 10px; cursor: pointer; }

        @media (orientation: landscape) { .scene { width: 550px; height: 260px; } .face { flex-direction: row !important; } .stub { width: 30% !important; height: 100% !important; } .perfor { width: 2px !important; height: 100% !important; border-left: 5px dotted #ccc !important; border-top: none !important; } .main { width: 70% !important; } .ticker { width: 550px; } }
    </style>
</head>
<body onclick="handleFlip(event)">
    <div id="ui">
        <input type="text" id="in" placeholder="ENTER LOCATION">
        <button onclick="startSearch()">SCAN</button>
    </div>
    <div class="scene" id="card">
        <div class="face front">
            <div class="stub" id="stb">
                <div style="font-size:7px; font-weight:900; opacity:0.7;">RADAR SCANNING</div>
                <div style="font-size:10px; font-weight:900; margin-top:5px;" id="airl">SEARCHING...</div>
                <div style="font-size:65px; font-weight:900; letter-spacing:-4px; margin:2px 0;">19A</div>
                <div class="dots-container">
                    <div id="d1" class="sq"></div><div id="d2" class="sq"></div><div id="d3" class="sq"></div><div id="d4" class="sq"></div><div id="d5" class="sq"></div>
                </div>
            </div>
            <div class="perfor"></div>
            <div class="main">
                <div style="color: #333; font-weight: 900; font-size: 11px; border: 1.5px solid #333; padding: 2px 8px; border-radius: 4px; align-self: flex-start;">BOARDING PASS</div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-top:10px;">
                    <div><span id="lb-1" style="font-size: 7px; font-weight: 900; color: #bbb;">AIRCRAFT ICAO</span><div id="f-icao" class="flap"></div></div>
                    <div><span id="lb-2" style="font-size: 7px; font-weight: 900; color: #bbb;">DISTANCE</span><div id="f-dist" class="flap"></div></div>
                    <div><span style="font-size: 7px; font-weight: 900; color: #bbb;">CALLSIGN</span><div id="f-call" class="flap"></div></div>
                    <div><span style="font-size: 7px; font-weight: 900; color: #bbb;">ROUTE</span><div id="f-route" class="flap"></div></div>
                </div>
                <div style="display:flex; justify-content:space-between; align-items:flex-end;">
                    <div id="arr" style="font-size:45px; transition:1.5s;">✈</div>
                    <div style="text-align:right;">
                        <div id="f-date" style="font-size:9px; font-weight:900; color:var(--blue-txt);">-- --- ----</div>
                        <div id="f-time" style="font-size:24px; font-weight:900;">--.--</div>
                        <img id="bc" src="https://bwipjs-api.metafloor.com/?bcid=code128&text=WAITING">
                    </div>
                </div>
            </div>
        </div>
        <div class="face back">
            <div style="height:100%; border:1px dashed #ccc; border-radius:15px; padding:20px; display:flex; flex-direction:column; position:relative;">
                <div style="display:flex; justify-content:space-between;">
                    <div><span style="font-size: 7px; font-weight: 900; color: #bbb;">ALTITUDE</span><div id="b-alt" class="flap"></div></div>
                    <div><span id="lb-3" style="font-size: 7px; font-weight: 900; color: #bbb;">G.SPEED</span><div id="b-spd" class="flap"></div></div>
                </div>
                
                <div class="gold-seal">OFFICIAL<br>COLLECTOR<br>AUTHENTIC</div>

                <div style="border: 3px double var(--blue-txt); color: var(--blue-txt); padding: 10px; border-radius: 10px; transform: rotate(-10deg); align-self: center; margin-top: 40px; text-align: center; font-weight: 900;">
                    <div style="font-size:8px;">SECURITY CHECKED</div>
                    <div id="b-date">-- --- ----</div>
                    <div id="b-time" style="font-size:22px;">--.--</div>
                </div>
            </div>
        </div>
    </div>
    <div class="ticker" id="tk">AWAITING LOCALIZATION...</div>

    <script>
        let pos = null, act = null, isTest = false, audioCtx = null, toggle = true;
        let tickerMsg = [], tickerIdx = 0;
        const chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ.- ";

        function playPing() {
            try {
                if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
                const osc = audioCtx.createOscillator(); const gain = audioCtx.createGain();
                osc.type = 'sine'; osc.frequency.setValueAtTime(880, audioCtx.currentTime);
                gain.gain.setValueAtTime(0.05, audioCtx.currentTime);
                gain.gain.exponentialRampToValueAtTime(0.0001, audioCtx.currentTime + 0.5);
                osc.connect(gain); gain.connect(audioCtx.destination);
                osc.start(); osc.stop(audioCtx.currentTime + 0.5);
            } catch(e) {}
        }

        function applyFlap(id, text, isTicker = false) {
            const el = document.getElementById(id); if(!el) return;
            const target = text.toUpperCase().padEnd(isTicker ? 32 : 8, ' ');
            el.innerHTML = '';
            [...target].forEach(c => {
                const s = document.createElement('span'); if(!isTicker) s.className = 'char';
                s.innerHTML = '&nbsp;'; el.appendChild(s);
                let cnt = 0, limit = 10 + Math.floor(Math.random()*20);
                const itv = setInterval(() => {
                    s.innerText = chars[Math.floor(Math.random()*chars.length)];
                    if(cnt++ >= limit) { clearInterval(itv); s.innerText = c === ' ' ? '' : c; }
                }, 40);
            });
        }

        // Alternância de labels a cada 15s
        setInterval(() => {
            if(act) {
                toggle = !toggle;
                document.getElementById('lb-1').innerText = toggle ? "AIRCRAFT ICAO" : "REGISTRATION";
                applyFlap('f-icao', toggle ? act.icao : act.reg);
                document.getElementById('lb-2').innerText = toggle ? "DISTANCE" : "EST. CONTACT";
                applyFlap('f-dist', toggle ? act.dist + "KM" : "ETA " + act.eta + "M");
                document.getElementById('lb-3').innerText = toggle ? "G.SPEED" : "AIRSPEED KTS";
                applyFlap('b-spd', toggle ? act.spd + "KH" : act.kts + "KT");
            }
        }, 15000);

        function updateTicker() { if(tickerMsg.length) { applyFlap('tk', tickerMsg[tickerIdx], true); tickerIdx = (tickerIdx+1)%tickerMsg.length; } }
        setInterval(updateTicker, 8000);

        async function update() {
            if(!pos) return;
            try {
                const r = await fetch(`/api/radar?lat=${pos.lat}&lon=${pos.lon}&test=${isTest}&_=${Date.now()}`);
                const d = await r.json();
                if(d.flight) {
                    const f = d.flight;
                    const card = document.getElementById('card');
                    
                    if(f.rare) card.classList.add('rare-active'); else card.classList.remove('rare-active');
                    document.getElementById('stb').style.background = f.color;

                    if(!act || act.icao !== f.icao) {
                        playPing();
                        document.getElementById('airl').innerText = f.airline;
                        applyFlap('f-call', f.call); applyFlap('f-route', f.route);
                        applyFlap('b-alt', f.alt + "FT");
                        document.getElementById('bc').src = `https://bwipjs-api.metafloor.com/?bcid=code128&text=${f.icao}`;
                        
                        // Gravação de Histórico de Selos (Apenas Reais e Raros)
                        if(!d.isTest && f.rare) {
                            let s = JSON.parse(localStorage.getItem('rare_seals') || '[]');
                            if(!s.includes(f.icao)) { s.push(f.icao); localStorage.setItem('rare_seals', JSON.stringify(s)); }
                        }
                    }
                    
                    document.getElementById('f-date').innerText = f.date; document.getElementById('f-time').innerText = f.time;
                    document.getElementById('b-date').innerText = f.date; document.getElementById('b-time').innerText = f.time;
                    for(let i=1; i<=5; i++) document.getElementById('d'+i).className = f.dist <= (190-(i-1)*40) ? 'sq on' : 'sq';
                    document.getElementById('arr').style.transform = `rotate(${f.hd-45}deg)`;
                    
                    const vStat = f.vrate > 200 ? "CLIMBING" : (f.vrate < -200 ? "DESCENDING" : "LEVEL");
                    tickerMsg = [f.rare ? "RARE MILITARY CONTACT" : "COMMERCIAL TRAFFIC", `V.RATE: ${f.vrate} FPM`, `STATUS: ${vStat}`, `WEATHER: ${d.weather.temp} ${d.weather.sky}`];
                    act = f;
                } else if (act) {
                    tickerMsg = ["SIGNAL LOST", "RESCANNING..."];
                }
            } catch(e) {}
        }

        function startSearch() {
            const v = document.getElementById('in').value.toUpperCase();
            if(v === "TEST") { isTest = true; pos = {lat:-22.9, lon:-43.1}; hideUI(); }
            else { fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${v}`).then(r=>r.json()).then(d=>{ if(d[0]){ pos={lat:parseFloat(d[0].lat), lon:parseFloat(d[0].lon)}; hideUI(); }}); }
        }
        function handleFlip(e) { if(!e.target.closest('#ui')) document.getElementById('card').classList.toggle('flipped'); }
        function hideUI() { document.getElementById('ui').classList.add('hide'); setTimeout(() => { update(); setInterval(update, 15000); }, 800); }
    </script>
</body>
</html>
''')

if __name__ == '__main__':
    app.run(debug=True)
