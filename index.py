...
# Configurações V98.0 - Full Fidelity & Rare Bird Test
RADIUS_KM = 190 
DEFAULT_LAT = -22.9068
DEFAULT_LON = -43.1729
...
def radar():
    try:
        ...
        if test:
            # MIX DE TESTE ALEATÓRIO PARA VALIDAÇÃO VISUAL
            test_aircrafts = [
                {"icao": "AE0802", "reg": "06-6164", "call": "RCH144", "airline": "US AIR FORCE", "color": "#1a1d21", "dist": 12.4, "alt": 32000, "spd": 830, "hd": 245, "route": "RMS-DOV", "vrate": -512},
                {"icao": "E48021", "reg": "PR-GXP", "call": "GLO1502", "airline": "GOL", "color": "#FF6700", "dist": 45.8, "alt": 35000, "spd": 780, "hd": 180, "route": "GIG-CGH", "vrate": 0},
                {"icao": "E4953E", "reg": "PT-MDS", "call": "TAM3010", "airline": "LATAM", "color": "#E6004C", "dist": 8.5, "alt": 3000, "spd": 380, "hd": 320, "route": "SDU-GRU", "vrate": -1200}
            ]
            f = random.choice(test_aircrafts)
            f.update({"date": now_date, "time": now_time, "kts": int(f['spd']/1.852), "eta": round(f['dist']/12)})
            ...
            for s in data:
                ...
                if slat and slon:
                    ...
                    if d <= RADIUS_KM:
                        ...
                        elif s.get('mil') == "1" or s.get('t') in ['C17', 'B52', 'F16']: airline, color = "MILITARY", "#1a1d21"
                        
                        spd_kts = int(s.get('gs', 0) or 0)
                        spd_kmh = int(spd_kts * 1.852)
                        eta = round((d / (spd_kmh or 1)) * 60)
                        proc.append({"icao": s.get('hex', 'UNK').upper(), "reg": s.get('r', 'N/A').upper(), "call": call, "airline": airline, "color": color, "dist": round(d, 1), "alt": int(s.get('alt_baro', 0) if s.get('alt_baro') != "ground" else 0), "spd": spd_kmh, "kts": spd_kts, "hd": int(s.get('track', 0) or 0), "date": now_date, "time": now_time, "route": s.get('route', "--- ---"), "eta": eta, "vrate": int(s.get('baro_rate', 0) or 0)})
            if proc: found = sorted(proc, key=lambda x: x['dist'])[0]
        return jsonify({"flight": found, "weather": w, "date": now_date, "time": now_time})
    except Exception as e: return jsonify({"flight": None, "error": str(e)})
...
<body onclick="handleFlip(event)">
        ...
        <div class="face back">
            <div style="height:100%; border:1px dashed #ccc; border-radius:15px; padding:20px; display:flex; flex-direction:column;">
                ...
                <div style="border: 3px double var(--blue-txt); color: var(--blue-txt); padding: 10px; border-radius: 10px; transform: rotate(-10deg); align-self: center; margin-top: 20px; text-align: center; font-weight: 900;">
                    ...
                    <div style="font-size:8px; margin-top:5px;">RADAR CONTACT V98.0</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let toggleState = true, isGhost = false, tickerMsg = [], tickerIdx = 0, audioCtx = null;
        let fDate = "-- --- ----", fTime = "--.--";
        ...
        setInterval(() => { if (tickerMsg.length > 0) { applyFlap('tk', tickerMsg[tickerIdx], true); tickerIdx = (tickerIdx + 1) % tickerMsg.length; } }, 6000);
        ...
        async function update() {
            ...
            try {
                ...
                if(d.flight) {
                    ...
                    let proximity = "MAINTAINING", vStatus = "LEVEL";
                    if(prevDist !== null) {
                        if(f.dist < prevDist - 0.05) proximity = "CLOSING IN";
                        else if(f.dist > prevDist + 0.05) proximity = "MOVING AWAY";
                    }
                    prevDist = f.dist;
                    if(f.vrate > 150) vStatus = "CLIMBING"; else if(f.vrate < -150) vStatus = "DESCENDING";
                    ...
                    act = f; tickerMsg = [`V.RATE: ${f.vrate} FPM`, vStatus, proximity, `SKY: ${weather.sky}`];
                } else if(act) {
                    isGhost = true; prevDist = null;
                    document.getElementById('stb').style.background = "var(--brand)";
                    tickerMsg = ["SIGNAL LOST", "GHOST MODE ACTIVE", `LAST POS: ${act.icao}`];
                    for(let i=1; i<=5; i++) document.getElementById('d'+i).className = 'sq';
                } else {
                    tickerMsg = ["SEARCHING TRAFFIC...", `TEMP: ${weather.temp}`, weather.sky];
                }
            } catch(e) {}
        }
        ...
        navigator.geolocation.getCurrentPosition(p => { pos = {lat:p.coords.latitude, lon:p.coords.longitude}; hideUI(); }, () => { applyFlap('tk', 'ENTER LOCATION', true); }, { timeout: 6000 });
    </script>
</body>
</html>
''')
