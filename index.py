...
# Configurações V118 - Military Heritage & Gold Seal Logic
RADIUS_KM = 190 
DEFAULT_LAT = -22.9068
DEFAULT_LON = -43.1729
...
def fetch_adsb_data(lat, lon):
    """Busca dados de voo com redundância de endpoints para evitar quedas."""
    endpoints = [
        f"https://api.adsb.lol/v2/lat/{lat}/lon/{lon}/dist/250",
        f"https://opendata.adsb.fi/api/v2/lat/{lat}/lon/{lon}/dist/250"
    ]
    headers = {'User-Agent': 'RadarSystem_V118/Military_Edition'}
...
@app.route('/api/radar')
def radar_engine():
    """Motor principal de processamento de alvos e filtragem de raridades."""
    try:
        ...
        if is_test:
            # Modo Teste com Blackbird e Comercial
            choice = random.choice(['MIL', 'COM'])
            if choice == 'MIL':
                return jsonify({
                    "flight": {
                        "icao": "SR71-X", "reg": "61-7972", "call": "BLACKBIRD", "airline": "MILITARY OPS",
            ...
            return jsonify({
                "flight": {
                    "icao": "AFE231", "reg": "PT-MDS", "call": "TEST777", "airline": "LOCAL TEST",
                    "color": "#34a8c9", "dist": 15.5, "alt": 35000, "spd": 850, "hd": 120,
                    "date": date_str, "time": time_str, "route": "GIG-MIA", "eta": 5,
                    "kts": 459, "vrate": -1500, "mil": False
                }, "weather": weather, "is_test": True
            })
...
<head>
    ...
    <style>
        :root { --gold: #D4AF37; --bg: #0b0e11; --brand: #000; --blue-txt: #34a8c9; }
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        /* Lateral Esquerda em Preto e Dourado */
        .stub { height: 32%; background: #000; color: var(--gold); padding: 25px; display: flex; flex-direction: column; justify-content: center; border-bottom: 2px solid var(--gold); position: relative; }
        .stub::after { content: "SAC"; position: absolute; top: 10px; right: 15px; font-size: 8px; opacity: 0.5; letter-spacing: 2px; }
        .dots { display: flex; gap: 5px; margin-top: 10px; }
        .sq { width: 12px; height: 12px; border: 1.5px solid rgba(212, 175, 55, 0.3); border-radius: 2px; transition: 0.4s; }
        ...
        .gold-seal::before { content: "☆ ☆ ☆"; display: block; margin-bottom: 4px; }
        ...
        small { font-size: 8px; color: #bbb; text-transform: uppercase; font-weight: 800; display: block; }
...
<body onclick="handleFlip(event)">
    ...
    <div class="scene" id="card">
        <div class="face front">
            <div class="stub" id="stb">
                ...
                <div style="font-size:68px; font-weight:900; margin:0; letter-spacing:-4px; color:#fff;">19A</div>
                <div class="dots">
            ...
            <div class="main">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 10px;">
                    ...
                    <div><small>OPERATIONAL STATUS</small><div id="f-route" class="flap-row"></div></div>
                </div>
                <div style="display:flex; justify-content: space-between; align-items: flex-end;">
                    <div id="arr" style="font-size:48px; transition: 1.5s cubic-bezier(0.17, 0.67, 0.83, 0.67);">✈</div>
        ...
        <div class="face back">
            ...
            <div style="border: 1px dashed #ccc; height:100%; padding: 20px; border-radius: 12px; display:flex; flex-direction:column; justify-content:space-between;">
                <div>
                    <small>ALTITUDE (BARO)</small><div id="b-alt" class="flap-row"></div>
                    <small style="margin-top:15px;">GROUND SPEED</small><div id="b-spd" class="flap-row"></div>
                </div>
                <div style="text-align:center; border: 4px double var(--blue-txt); color:var(--blue-txt); padding:15px; transform: rotate(-8deg); font-weight:900; border-radius: 10px;">
                    ...
                    <div style="font-size:8px; margin-top:4px; opacity:0.6;">SYSTEM V118 SECURE</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let pos = null, act = null, isTest = false, toggle = true, tickIdx = 0;
        let audioCtx = null;
        ...
        function playTick() {
            try {
                ...
                o.type = 'sine'; o.frequency.setValueAtTime(800 + Math.random()*200, audioCtx.currentTime);
        ...
        function applyFlap(id, text) {
            ...
            const size = 8; const target = text.toUpperCase().padEnd(size, ' ').substring(0, size);
            ...
            [...target].forEach((letter, i) => {
                ...
                if(slot.innerText !== letter) {
                    ...
                    const timer = setInterval(() => {
                        ...
                        if(iterations++ > 15) {
                            slot.innerText = letter; clearInterval(timer);
                            playTick();
                        }
                    }, 40);
                }
            ...
            try {
                ...
                if(d.flight) {
                    ...
                    if(!act || act.icao !== f.icao) {
                        ...
                        // Lógica de Selo e Histórico
                        if(f.mil) {
                            document.getElementById('seal').style.display = 'flex';
                            if(!d.is_test) {
                                let history = JSON.parse(localStorage.getItem('rare_history') || '[]');
                                if(!history.includes(f.icao)) { history.push(f.icao); localStorage.setItem('rare_history', JSON.stringify(history)); }
                            }
                        } else { document.getElementById('seal').style.display = 'none'; }
                    }
                    ...
                    const msgs = [`V.RATE: ${f.vrate} FPM`, f.mil ? "RARE ASSET IDENTIFIED" : "CONTACT ACTIVE", `TEMP: ${d.weather.temp} | SKY: ${d.weather.sky}`];
                    document.getElementById('tk').innerText = msgs[tickIdx % 3];
                    tickIdx++; act = f;
                } else {
                    document.getElementById('tk').innerText = "SEARCHING TRAFFIC...";
